 ...
 */
 (function($) {
	$.previewImage = function(options) {
	    ...
	    var wid = $("#preview-image-plugin-overlay").css("width");
	    var heig = $("#preview-image-plugin-overlay").css("height");
	    
	    console.log("width="+wid+" / height="+heig);
	    
		var opts = $.extend({
			/* The following set of options are the ones that should most often be changed 
			   by passing an options object into this method.
			*/
			'xOffset': 2,    // the x offset from the cursor where the image will be overlayed.
			'yOffset': 2,   // the y offset from the cursor where the image will be overlayed.			
