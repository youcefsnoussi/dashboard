...
$(document).ready(function () {
  ...
  // ===================== PARENT ARTICLE MAPPING =====================
  loadParentArticleSelect();

  $('#parent_article_select').change(function () {
    $('#parent_article_id').val($(this).val());
  });

  if ($('#article').val() != '') {
    $('#frm').attr('action', 'edit_article');

    // Load parent mapping for this article
    loadParentMapping($('#id_art').val());
...
function loadParentArticleSelect() {
  $.ajaxSetup({ async: false });
  $.ajax({
    url: 'get_all_articles_for_parent',
    success: function (responseJson) {
      if (responseJson != null) {
        var currentArticleId = $('#id_art').val();
        $.each(responseJson, function (key, value) {
          if (value.id != currentArticleId) {
            $('#parent_article_select').append(
              '<option value="' +
                value.id +
                '">' +
                value.code +
        ...
        $('#parent_article_select').selectpicker('refresh');
      }
    },
  });
}

function loadParentMapping(articleId) {
  $.ajaxSetup({ async: false });
  $.ajax({
    url: 'getParentMapping',
    data: { articleId: articleId },
    success: function (response) {
      if (
        response != null &&
        response.parentArticleId != null &&
        response.parentArticleId > 0
      ) {
        $('#parent_article_select').val(response.parentArticleId);
        $('#parent_article_id').val(response.parentArticleId);
        $('#parent_article_select').selectpicker('refresh');
      }
    },
  });
...
function update_sous_cat() {
  ...
  $.ajax({
    ...
    success: function (responseJson) {
      if (responseJson != 'null') {
        $.each(responseJson, function (key, value) {
          $('#sous_cat_prod').append(
            '<option value="' +
              value.id +
              '">' +
              value.nom_sous_category +
              '</option>',
          );
        });
      }
